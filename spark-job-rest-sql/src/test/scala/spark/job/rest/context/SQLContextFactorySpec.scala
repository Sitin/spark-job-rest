...
class SQLContextFactorySpec extends WordSpec with MustMatchers with BeforeAndAfter {
  ...
  val sqlContextFactoryConfig = ConfigFactory.parseString(
    """
      ...
      |spark.job.rest {
      |  context.jars = []
      |  context.job-context-factory = "spark.job.rest.context.SparkSQLContextFactory"
      |}
    """.stripMargin).resolve()
  ...
  val hiveSqlFactoryWithCustomSparkContextConfig = ConfigFactory.parseString(
    """
      ...
      |spark.job.rest {
      |  context.jars = []
      |  context.job-context-factory = "spark.job.rest.context.HiveContextFactory"
      |  context.spark-context-factory = "spark.job.rest.context.FakeJobContextFactory"
      |}
    """.stripMargin).resolve()
}
