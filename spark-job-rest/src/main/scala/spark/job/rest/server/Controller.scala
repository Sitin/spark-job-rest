...
class Controller(val config: Config,
  ...
  /**
   * Database connection received from connection provider [[spark.job.rest.server.domain.actors.DatabaseServerActor]]
   */
      ...
      delete {
        path(Segment) { contextName =>
          corsFilter(List("*")) {
            ...
            respondWithMediaType(MediaTypes.`application/json`) { ctx =>
              resultFuture.map {
                case _: Success[_] => ctx.complete(StatusCodes.OK, SimpleMessage(s"Context $contextName deleted."))
                case NoSuchContext => ctx.complete(StatusCodes.BadRequest, ErrorResponse("No such context."))
                case x: Any => ctx.complete(StatusCodes.InternalServerError, ErrorResponse(x.toString))
              }
            }
